...
dayS: DayT \wss{You use this type, but I don't think you import (use) the module
  that defines it}\\
dayE: DayT
...
DayDurT = ? \wss{It seems confusing to me that you have a DayT, that is not an
...
\subsubsection{State Variables}
dayS : DayT \wss{You have these same state variables in another module}\\
dayE : DayT\\
...
\subsubsection{State Variables}
latitude: DegreeT\\
dayS: DayT \wss{You have these state variables already in other modules.}\\
dayE: DayT\\
panH: real\\
panW real
...
\item transition: resultL := $\|$ ($|$resultL$|$ = 0 $\Rightarrow$ [ $\langle$
  ...
  $\langle$ angle, energy, getdiff (resultL.e, energy) $\rangle$ ] ) \wss{I
