...
day : DayT \wss{You have these same state variables in another module}\\
...
\begin{itemize}
\item transition:  day = (m = 1 $\lor$ m = 2 $\Rightarrow $ 365.25 $\times$ (y - 1) + 30.6001 $\times$ (m + 13) + d - calculateB(y/100) +  1720995 $|$ True $\Rightarrow $ 365.25 $\times$ y + 30.6001 $\times$ (m + 1) + d - calculateB(y / 100) +  1720995)\\
...
\begin{itemize}
\item transition: 
\item output: out := compare ( day, inday )
\item exception:
\end{itemize}
...
\begin{itemize}
\item transition: 
\item output: out := compare ( inday, day, )
\item exception:
\end{itemize}
...
setdurlist ( staDay, inpDay ): = ( staDay $\leq$ inpDay $\Rightarrow $ daduraL [0 .. $|$daduraL$|$] $\|$ $\langle$ countDiff (perihelion ( staDay ), staDay) $\rangle$ $\|$ setdurlist (getnext (staDay), inpDay)) $|$ staDay $>$ inpDay $\Rightarrow $ daduraL [0 .. $|$daduraL$|$] )\\

calculateB: $\mathbb{N}$ $\rightarrow $ $\mathbb{N}$\\
calculateB (a) = 2 - a + (a / 4)\\


compare: DayT $\times$ DayT $\rightarrow $ $\mathbb{B}$\\
...
countDiff (day1, day2) := (day1 $\leq$  day2 $\Rightarrow$ 1 + countDiff (getnext (day1), day2) $|$ day1 $<$  day2 $\Rightarrow$ 0)
...
\subsubsection{State Variables}
resultAEng: a sequence of a tuple \wss{spell check}\an{yes} of ( t: DegreeT, e: real, diff: integer)\\
