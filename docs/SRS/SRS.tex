...
linkcolor=red, % color of internal links (change box color with linkbordercolor)citecolor=green, % color of links to bibliography
...
\midrule 
$\theta_{S_{\text{day}}}$ & \si[per-mode=symbol] {\degree} & zenith angle of
sun in the day
  \\
  ...
  $\Phi_P$ & \si[per-mode=symbol] {(\degree, \ ')} & the latitude
   of the solar panel \wss{the units are degrees. N and S are
    ...
    will help you.  I'll include some details below.} \an{Yes, I it fixed by treating the N and S
    as + and - }
  \\
$\delta_{\text{date}}$ & \si[per-mode=symbol] {(\degree, \ N/S)} & the daily declination of
the vertical noon sun in the date duration
  ...
  \\
  $I_{S_{\text{daily}}}$ & \si[per-mode=symbol] {\frac{k\watt}{\square{m}}} & the daily 
solar intensity of the noon in the date duration($date$).
  \\
  $I_{S_{\text{total}}}$ & \si[per-mode=symbol] {\frac{k\watt}{\square{m}}} & the total 
solar intensity of the noon in the date duration($date$).
  \\
  $I_{S_{\text{max}}}$ & \si[per-mode=symbol] {\frac{k\watt}{\square{m}}} & the hightest 
solar intensity of the noon in the date duration($date$).
  \\
  $P_{E}$ & \si[per-mode=symbol] {k\watt} &the estimated solar panel output
  energy
  \\
  $P_{A_{\text{h}}}$ &\si[per-mode=symbol] {\square{m}} & the height of the solar panel
  \\
  $P_{A_{\text{w}}}$ &\si[per-mode=symbol] {\square{m}} &the width of the solar panel 
  \\
  ...
  \\
  $date$ & \si[per-mode=symbol] {\text{-}} & the date duration between the 
  calculation starting date and the ending date
  \\
  $\mathit{year}_\text{Start}$& \si[per-mode=symbol] {\text{-}} & the year of the
  calcuation's starting date\\
  $\mathit{month}_\text{Start}$& \si[per-mode=symbol] {\text{-}} & the month of
  the calcuation's starting date\\ 
  $\mathit{day}_\text{Start}$ & \si[per-mode=symbol] {\text{-}} & the day of the
  calcuation's starting date\\
  $\mathit{year}_\text{End}$ & \si[per-mode=symbol] {\text{-}} & the year of the
  calcuation's ending date\\
  $\mathit{month}_\text{End}$& \si[per-mode=symbol] {\text{-}} & the month of the
  calcuation's ending date\\
\bottomrule
\end{longtable*}
...
Specification (SRS) for \progname(SC). \wss{remember to check with the
writingchecklists!} This section explains the purpose and the organization of
the
...
your tex
  file.  It is better for diffs, and easier to read on different editors.}
...
with no trees, hills, clouds, dust, or haze ever blocking the sun. \wss{Clarify
that for the purpose of the calculations you are making these assumptions.
  The actual solar panel will never reach the ideal energy output given in the
  calculations by \progname.}  \wss{I tried to use \progname{}, but you haven't
  redefined it to SC.   You should do tha that in the Common.tex file.}
...
itcan
ignore the effect of temperature (or pressure) on the material
  properties, etc.}  
...
This section summarized the expectation of the readers' knowledge and skills
forunderstanding this SRS. Readers should have a general knowledge of how a
solar
panel works and know the common factors that affect energy absorption.
\wss{Usually these characteristics are given in terms of education level, and
  ...
  should understand the longitude and latitude coordinate system.}
...
\item Ensure the input date format matches the requirement from SC.  \wss{I
    think you can just say ``Ensure the input is formatted correctly.''}
\item Ensure the condition of the solar panel, and the surrounding environment
  of the solar panel. \wss{The responsibilities are for using the software, not
    for using a solar panel.  I don't think you need this responsibility.}
\end{itemize}
\item SC Responsibilities:
\begin{itemize}
\item Detect data type mismatch, such as a string of characters instead of a
  floating point number.
\item Comfirm \wss{spellcheck!} the inputs quality to satisfy the required
physical and software constraints.
\item Predict an optimum tilt angle and the expected gaining solar energy, and
  show the result explicitly to users (such as generating a report with linear
  graphic ). \wss{You can write this more abstractly.  The specifics of the
...
such as angle addition and subtraction theorems and understand the Level 1
celestial
mechanics such as the formation of the latitude and the earth's tilt angle in
the four seasons.  \wss{I don't think Level 1 Calculation and Level 1 celestial
...
\item Tilt angle: The angle for adjusting the solar panel result in the panel
  get the direct sunlight. According to the Figure:\nameref{Fig_PhysicFigure},
  tilt angle is equal to the zenith angle.  \wss{I think you mean that for
...
solar panel that has a different tilt angle from the zenith angle.}

\item The period of days: The period of days depends on the input days of the
users.
...
\item Latitude: The angle which ranges from 0 $^\circ$ at the Equator to
  90$^\circ$ (North or South) at the poles.

\item Zenith Angle: The angle between the sun and the vertical. According to
theFigure:\nameref{Fig_PhysicFigure}, zenith angle is equal to the tilt angle.
  \wss{Please see my comment under tilt angle.}
...
panel($P_A$), the day started to estimate angle($day_{Start}$),the day when the
estimation end($day_{End}$), the goal statements are: \wss{you should have
  spaces before opening brackets.  Please see the document review checklists.}

\begin{itemize}

\item[GS\refstepcounter{goalnum}\thegoalnum \label{G_meaningfulLabel}:] Predict
...
sense.}\subsubsection{Theoretical Models}\label{sec_theoretical}
...
\partial A}{\cos{\theta} \times \partial\Omega_0 \times \partial A_0}$\\
  ...
  Description & 
			In the following source, Lambert's cosine law state that
                "the radiant intensity or luminous intensity observed from an
                ideal diffusely reflecting surface or ideal diffuse radiator is
                directly proportional to the cosine of the angle $\theta$
                between the direction of the incident light and the surface
                normal." \wss{look at the writing checklist for a discussion of
                opening and closing quotation marks.}\\
			&$I_0$ is the radiance of illumination.\\
			&I is the number of the photons. \wss{When you show a
                   symbol outside of an equation, it should be formatted as in
                   the equation.  In this case $I$.}\\
...
\frac{1.00}{I_{S}}^ {sec(\theta_{S_{\text{day}}})}$\\
		Where $\frac{1}{\cos{\theta_{S_{\text{day}}}}}$ is $sec(\theta_{S_{\text{day}}})$\\
...
% \hline
  ...
  \hline
  Description &
'date' defines in the table: \nameref{tb_TS} and refer from \ddref{C_D}.\\&The
degree of $\delta_{\text{date}} $ can be determinated by
Figure: \nameref{Fig_Analemma}.\\
  \hline
Sources&
\url{https://www.cengage.com/resource_uploads/downloads/0495555061_137179.pdf}\\\hline
  Ref.\ By &  \ddref{C_ZA}\\
  \hline
\end{tabular}
\end{minipage}\\
...
Equation&  \[
    \theta_{S_{\text{day}}}= 
\begin{cases}
\Phi_P - \delta_{\text{date}} ,& \text{if } 0 \leq \Phi_P, \delta_{\text{date}} \leq 90^\circ
\text{N } \lor\ 0 \leq \Phi_P, \delta_{\text{date}} \leq 90^\circ \text{S }\\
    \Phi_P + \delta_{\text{date}} ,& \text{if } \begin{cases} 
0 \leq \Phi_P\leq 90^\circ \text{N } \land\ 0 \leq \delta_{\text{date}} \leq 90^\circ
\text{S } \\
0 \leq \Phi_P\leq 90^\circ \text{S } \land\ 0 \leq \delta_{\text{date}} \leq 90^\circ
\text{N }
      \end{cases}
\end{cases}
\]\\
...
panel.\\&$\theta_{S_{\text{day}}} $ is the zeith angle of the sun in the day.\\
&$\Phi_P$ is the local latitude of the solar panel from the users' input
described at \nameref{sec_GS}.\\
&$\delta_{\text{date}} $ is the declination of the vertical noon sun in the day. It is
...
\hline
  ...
  \hline
  Description &
$day_{Start}$, $day_{End}$ are the days depending on the users' input described
at \nameref{sec_GS}.\\
...
\wss{Adding a data type for date would help make this clearer.}
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  Input&$\theta_{S_{\text{day}}},I_{S_{date}}$\\

  \hline
  Output & $ \theta_{T} , I_{S_{\text{max}}}$\\
& $I_{S_{\text{max}}}  = I_{S} \cdot \frac{1.00}{I_{S_{\text{total}}}}^ {sec(\theta_{S_{\text{day}}})}
                                         \text{where} \ \theta_{T} \ \text{is
                                         the valuse that has the hightest} \
I_{S_{\text{max}}}$ \wss{write this using argmax
\url{https://en.wikipedia.org/wiki/Arg_max}.}\\
  \hline
  Description&
Using General Definition: \dref{C_SI} can get the total solar intensity
$I_{S_{date}}$ during the period od date. Then using the value $I_{S_{date}}$
recalculates the optimum angle $\theta_{T}$. \\
		&$I_{S_{date}}$ is input from \dref{C_SI}.\\
		&$\theta_{S_{\text{day}}}$ is input from \ddref{C_DS}.
\\
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  Input&$\theta_{S_{\text{day}}},I_{S_{\text{max}}}$\\

  \hline
  Output & $ I_{S_{\text{daily}}}$\\
& $I_{S_{\text{daily}}} = I_{S} \cdot \frac{1.00}{I_{S_{\text{max}}}}^ {sec(\theta_{S_{\text{day}}})}
$\\
  \hline
  Description&
...
\textbf{Typical Value} & \textbf{Uncertainty}\\
  \midrule 
$\Phi_P$ & $- 90^\circ \leq \Phi_P \leq 90^\circ $ & $\text{-}$ & {$(43^\circ, 15'39.3")$} & 10\%\\
...
requested by the system, which is the latitude of the solar panel, the date
starts the calculation and the ending date.\\
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_Calculate}:] Determine the
tilt angle
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_VerifyOutput}:] Verify the
output
by using the Table: $\nameref{TblOutputVar}$\\
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_Output}:]Reading the section
of
$\nameref{sec_assumpt}$ to know the related requirements regarding the outputs.
\wss{I do not understand this requirement.}\\
