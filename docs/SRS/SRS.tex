...
the N and S
    as + and - }
  \\
$\delta_{\text{date}}$ & \si[per-mode=symbol] {(\degree, \ N/S)} & the daily
declination of
...
solar panel works and know the common factors that affect energy absorption.
...
&$(\theta_{S_{\text{day}}})$ is the zenith angle of the sun in the day. It is
input
from $\ddref{C_ZA}$.\\
...
Where $\frac{1}{\cos{\theta_{S_{\text{day}}}}}$ is
$sec(\theta_{S_{\text{day}}})$\\
...
& $I_{S_{\text{max}}} = I_{S} \cdot \frac{1.00}{I_{S_{\text{total}}}}^
{sec(\theta_{S_{\text{day}}})}
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_Outputshowg}:] \progname~should able to show the comparison
between the outputs corresponding to the different user's input in a graphic.\\
\wss{I do not understand this requirement.} \an{Changed the original requirement}\\
