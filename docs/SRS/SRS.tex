...
%\begin{table}[ht]
  \noindent \begin{tabular}{l l l} 
    ...
    \si{'} & angle	& - \\
    \si{k} & mass   & kilo \wss{mass is in kg, kilo should be kilograms}\\
    \si{m} &  length   & metre \\
    \si{\watt} & power & Watt (W = \si{\joule\per\second})\\
	  N & direction & North \wss{N and S are not units}\\
	  S & direction & South\\
	  year & date & date \wss{defining a date type is a great idea, but
                        dates are not units}\\
	  month & date & date\\
	  day & date & date\\
    \bottomrule
...
\noindent \begin{longtable*}{l l p{12cm}} \toprule
  ...
  \midrule $\theta_{S_{day}}$ & \si[per-mode=symbol] {\degree} & zenith angle of
  sun in the day
  \\
  $\theta_{T}$ & \si[per-mode=symbol] {\degree} & the tilt angle for adjusting
  the solar panel
  \\
  $\Phi_P$ & \si[per-mode=symbol] {(\degree, \N/S, \ ')} & the latituade
  \wss{Spellcheck!} of the solar panel \wss{the units are degrees. N and S are
    not units.  I can see that this could be confusing.  Defining a data type
    will help you.  I'll include some details below.}
  \\
  $\delta_{date}$ & \si[per-mode=symbol] {(\degree, \ N/S)} & the declination of
  the vertical noon sun in the day
  \\
  $I_{S}$ & \si[per-mode=symbol] {\frac{k\watt}{\square{m}}} & the intensity of
  the sun measured by the satellites
  \\
  $I_{S_{date}}$ & \si[per-mode=symbol] {\frac{k\watt}{\square{m}}} & the
  intensity of the sun in a day of the noon
  \\
  $I_{S_{date}}$ & \si[per-mode=symbol] {\frac{k\watt}{\square{m}}} & the solar
  intensity in a period of days of the noon
  \\
  $I_{S_{T}}$ & \si[per-mode=symbol] {\frac{k\watt}{\square{m}}} & the solar
  intensity hits on the tilted solar panel.
  \\
  ...
  \\
  $date$ & \si[per-mode=symbol] {\text{-}} & a sequence of dates from the day
  starts estimating the angle to the ending day
  \\
  $day_{Start}$ & \si[per-mode=symbol] {\text{-}} & the day starts estimating
  the angle \wss{Please check with the Writing-Checklist.tex document for the
    format of variables that are words in LaTeX.  This should be
    $\mathit{day}_\text{Start}$.  I know it is tedious, but it doesn't look
    right otherwise.}
  \\
  $day_{End}$ & \si[per-mode=symbol] {\text{-}} & the day stops estimating the
  angle
  \\
  \bottomrule
\end{longtable*}
...
seasons. Therefore, Sun Catcher is created for home users to gain optimum energy
from daily sunlight.


The following sections provide an overview of the Software Requirements
Specification (SRS) for Sun Catcher(SC). \wss{remember to check with the writing
  checklists!}  This section explains the purpose and the organization of the
document, the scope of the requirements, and the characteristics of the intended
reader.  \wss{You should put use 80 character wide columns of text in your tex
...
This section summarized the expectation of the readers' knowledge and skills for
understanding this SRS. Readers should have a general knowledge of how a solar
...
\item Comfirm \wss{spellcheck!} the inputs quality to satisfy the required physical and software constraints.
...
Sun Catcher is intended to solve the unpredictable energy efficiency of solar
panels.  Due to the tilt angle of the earth when it rotates by axis, the
latitude of the sun will consistently move. With the fixed location of the solar
panel, it is unlikely to get the direct the direct sunlight for the maximum
output. Therefore, it causes inadequate performance in gaining energy.  \iffalse
\plt{What problem does your program solve?  The description here should be in
  the problem space, not the solution space.}\fi
...
\item Zenith Angle: The angle between the sun and the vertical. According to the
...
\wss{I suggest that you add a section for Type Definitions.  You can see an
  ...
  determine how they are represented.  The following web-page has a good summary
of the representation of longitude and latitude:
...
\wss{You should also think about a type for dates.  Again, a tuple makes sense.}
...
% \hline
  ...
  Description &
       ...
       &The degree of $\delta_{date} $ can be determinated \wss{spell check!} by Figure: \nameref{Fig_Analemma}.\\
  \hline
...
& $I_{S_{T}}  = I_{S} \cdot \frac{1.00}{I_{S_{date}}}^ {sec(\theta_{S_{day}})}
                                         \text{where} \ \theta_{T} \ \text{is
                                         the valuse that has the hightest} \
                                         I_{S_{T}}$ \wss{write this using argmax
...
\hline
	& \aref{a_spe}& \aref{a_dsip}& \aref{a_dsia} & \aref{a_dp_r} & \aref{a_dpr}& \aref{a_tilta}& \aref{a_panele} \\
\hline
